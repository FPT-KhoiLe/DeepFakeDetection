...
def print_confusion_matrix(y_true, y_pred, labels=["Fake", "Real"]):
...
def main():
    ...
    train_loader = DataLoader(train_data, batch_size=4, shuffle=True, num_workers=2, persistent_workers=True)
    valid_loader = DataLoader(val_data, batch_size=4, shuffle=True, num_workers=2, persistent_workers=True)
    ...
    print_confusion_matrix(early_stopping.best_metrics['Actual'],early_stopping.best_metrics['Predict'])
    end_time = time.time()
